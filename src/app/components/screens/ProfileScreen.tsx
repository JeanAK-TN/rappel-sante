...
const menuItems = [
  ...
  { icon: <Share2 size={18} color="#607D8B" />, label: "Partage de données", hasToggle: true, toggleOn: true },
  { icon: <Bell size={18} color="#607D8B" />, label: "Notifications" },
  { icon: <Globe size={18} color="#607D8B" />, label: "Langue", value: "Français" },
  { icon: <Wifi size={18} color="#607D8B" />, label: "Mode hors ligne", hasToggle: true, toggleOn: false },
      ...
      <div style={{ flex: 1, overflowY: "auto", paddingBottom: 90 }}>
        ...
        <div style={{ background: "#FFFFFF", padding: "20px 20px", display: "flex", alignItems: "center", gap: 16 }}>
          <div style={{ width: 64, height: 64, background: "#1E7D5C", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <span style={{ fontSize: 22, fontWeight: 700, color: "#FFFFFF" }}>KA</span>
          </div>
          <div>
            <div style={{ fontSize: 18, fontWeight: 700, color: "#1A2E3B" }}>Kofi Amewoyi</div>
            <div style={{ fontSize: 13, color: "#607D8B", marginBottom: 8 }}>42 ans · +228 90 23 45 67</div>
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
              {["Hypertension", "Diabète type 2"].map(p => (
              ...
              <div key={i} style={{ padding: "14px 16px", display: "flex", alignItems: "center", gap: 14, borderBottom: i < menuItems.length - 1 ? "1px solid #F4F6F7" : "none" }}>
                ...
                {item.hasToggle ? (
                  <div style={{
                    ...
                    background: item.toggleOn ? "#1E7D5C" : "#D1D5DB",
        ...
        <div style={{ marginInline: 20 }}>
          <div style={{ background: "#FFFFFF", borderRadius: 16, padding: "14px 16px", display: "flex", alignItems: "center", gap: 14, boxShadow: "0px 2px 12px rgba(0,0,0,0.06)" }}>
